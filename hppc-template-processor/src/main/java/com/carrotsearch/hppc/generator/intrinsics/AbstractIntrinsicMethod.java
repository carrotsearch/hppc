...
abstract class AbstractIntrinsicMethod implements IntrinsicMethod {
  protected static Type inferTemplateType(
    ...
    switch (templateCast) {
      ...
      default:
        throw new RuntimeException(
            format("Ukrecognized template cast to %s in: %s", templateCast, m.group()));
    }
    return type;
  }

  protected static String inferTemplateCastName(
      Matcher m, TemplateOptions templateOptions, String templateCast) {
    ...
    if (templateCast == null) {
      throw new RuntimeException(format("Couldn't infer template type of: %s", m.group()));
    }
    return templateCast;
  }

  protected static void expectArgumentCount(
      Matcher m, ArrayList<String> arguments, int expectedCount) {
    if (arguments.size() != expectedCount) {
      throw new RuntimeException(
          format(
              "Expected exactly %d arguments but was %d: %s(%s)",
              expectedCount, arguments.size(), m.group(), arguments));
    }
  }
