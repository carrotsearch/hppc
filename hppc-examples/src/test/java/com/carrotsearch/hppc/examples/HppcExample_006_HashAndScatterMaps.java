...
public class HppcExample_006_HashAndScatterMaps {
  ...
  public void scatterVsHashDifference() throws Exception {
    ...
    for (int i = 0; i < 50; i++) {
      ...
      println(
          "%5d keys, buffer size: %6d, occupancy: %s",
          set.size(), set.keys.length, set.visualizeKeyDistribution(30));
    }
    ...
    println(
        "%5d keys, buffer size: %6d, occupancy: %s",
        other.size(), other.keys.length, other.visualizeKeyDistribution(30));
    ...
    for (int v : set.toArray()) {
      ...
      if ((++added % 10000) == 0) {
        ...
        println(
            "%6d keys, round: %5d ms, buffer: %s",
            added, round, other.visualizeKeyDistribution(40));
