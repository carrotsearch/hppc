...
<xsl:template match="BugInstance[not(@last)]">
	...
	<tr class="tablerow{position() mod 2} href" onclick="toggleRow('{$warningId}');">
	...
	<tr class="detailrow{position() mod 2}">
		...
		<td>
			<p id="{$warningId}" style="display: none;">
				<a href="#{@type}">Bug type <xsl:value-of select="@type"/></a>
...
<xsl:template match="BugPattern">
	<h3><a name="{@type}"><xsl:value-of select="@type"/>: <xsl:value-of select="ShortDescription"/></a></h3>
