...
/**
 * A double-linked set of <code>int</code> values. This data structure is
 * characterized by constant-time lookup, insertions, deletions and removal of
 * all set elements (unlike a {@link BitSet} which takes time proportional to
 * the maximum element's length).
 * 
 * <p>
 * The implementation in based on <a
 * href="http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.30.7319">
 * Preston Briggs and Linda Torczon's paper
 * "An Efficient Representation for Sparse Sets"</a>
 * </p>
 */
             ...
             Cloneable
{
   ...
   * number of elements should be equal to the actual number of elements added
   * to the container. The sparse container's expected size should be at least
   * one more than the maximum value stored in the set.
   * 
   * @see BoundedProportionalArraySizingStrategy
   */
   ...
   * 
   * The maximum expected element's value is required to size sparse array
   * accordingly. If unknown, leave at a good guess or zero.
   * 
   * @param expectedElements
   *          The total number of elements, inclusive.
   * @param maxElementValue
   *          Maximum value of any element added to the set, inclusive.
   */
  ...
  protected void ensureDenseBufferSpace(int expectedAdditions) {
    ...
    if (elementsCount > bufferLen - expectedAdditions) {
      ...
      assert newSize >= elementsCount + expectedAdditions : 
            "Resizer failed to" + " return sensible new size: " + 
  ...
  /**
   * Ensures the internal sparse buffer has enough free slots to store index of
   * <code>value</code>.
   */
  ...
  public boolean contains(int value) {
    int index;
    return value >= 0 && value < sparse.length && (index = sparse[value]) < elementsCount && dense[index] == value;
  }
  ...
  /**
   * A faster version of {@link #add(int)} that does check or attempt to expand
   * the internal buffers. Assertions are still present.
   */
  protected void addNoChecks(int value) {
    assert value >= 0 : "Double linked set supports values >= 0 only.";
   ...
   * </p>
   * 
   * @return Returns the number of elements that were added to the set (were not
   *         present in the set).
   */
  ...
  public static DoubleLinkedIntSet from(int... elements) {
    final DoubleLinkedIntSet set = new DoubleLinkedIntSet(elements.length, 1 + maxElement(elements));
    for (int i : elements)
      set.addNoChecks(i);
    return set;
  }
